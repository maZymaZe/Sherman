...
constexpr int kInternalCardinality =
    (kInternalPageSize - sizeof(Header) - sizeof(uint8_t) * 2 -
     sizeof(uint64_t) - sizeof(uint8_t) * 3 - 1) /
...
class Tree {
   public:
    ...
    // void range_query(const Key &from, const Key &to, std::map<Key, Value>
    // &ret);
    uint64_t range_query(const Key& from,
                         const Key& to,
   ...
   private:
    ...
    static thread_local std::vector<GlobalAddress>
        scan_leaves[define::kMaxCoro];
    ...
    bool page_search(GlobalAddress page_addr,
                     ...
                     TmpResult* t_res = nullptr);
    bool level_one_page_search(const Key& k,
                               TmpResult* result,
                               CoroContext* cxt = nullptr,
                               int coro_id = 0);
