...
void Tree::insert(const Key& k, const Value& v, CoroContext* cxt, int coro_id) {
    ...
    if (enable_cache) {
        ...
        if (entry) {  // cache hit
            auto root = get_root_ptr(cxt, coro_id);
            if (leaf_page_store(cache_addr, k, v, root, 0, cxt, coro_id,
                                true)) {
...
bool Tree::level_one_page_search(const Key& k,
                                 TmpResult* t_res,
                                 CoroContext* cxt,
                                 int coro_id) {
    auto root = get_root_ptr(cxt, coro_id);
...
*/
// void Tree::range_query(const Key &from, const Key &to, std::map<Key, Value>
// &ret) {
...
//   读取被cache住的倒数第二层node cache_hit[dsm->getMyThreadID()] +=
//   result.size() * spanSize; int cnt = 0; auto tmp = from; while(tmp < to) cnt
//   ++, tmp = tmp + 1; cache_miss[dsm->getMyThreadID()] += cnt - result.size()
//   * spanSize;

//   std::sort(result.begin(), result.end(), [](const InternalPage& a, const
//   InternalPage& b){
//     return std::make_pair(a.hdr.lowest, a.hdr.highest) <
//     std::make_pair(b.hdr.lowest, b.hdr.highest);
//   });
...
//     if (level_one_page_search(result.back().hdr.highest, &tmp_res, nullptr,
//     0)
//         && tmp_res.level == 1)
...
//               sizeof(LeafPage), true, nullptr);  // poll cq in this batch
//               instead of at master corotine
//     cq_cnt++;
//   }
...
uint64_t Tree::range_query(const Key& from,
                           const Key& to,
    ...
    if (result.empty()) {
        for (auto k = from; k < to; k = k + spanSize)
    ...
    for (auto page : result) {
        ...
        for (int i = 1; i < cnt; ++i) {
            no_fetch =
                from > page.records[i].key || to < page.records[i - 1].key;
            if (!no_fetch) {
                leaves.push_back(page.records[i - 1].ptr);
            }
        }
...
re_read:
    ...
    dsm->read_sync(page_buffer, page_addr,
                   std::max(kLeafPageSize, kInternalPageSize), cxt);
    pattern_cnt++;
    ...
    if (result.is_leaf) {
        ...
        if (from_cache && (k < page->hdr.lowest ||
                           k >= page->hdr.highest)) {  // cache is stale
            return false;
        }
...
void Tree::internal_page_search(InternalPage* page,
    ...
    if (k >= page->hdr.highest) {  // should turn right  // BUG:
                                   // 很可能是这里导致的cache启动失败
        result.slibing = page->hdr.sibling_ptr;
        return;
    }
...
void Tree::leaf_page_search(LeafPage* page,
                            const Key& k,
                            SearchResult& result) {
    for (int i = 0; i < kLeafCardinality; ++i) {
        auto& r = page->records[i];
...
bool Tree::leaf_page_store(GlobalAddress page_addr,
    ...
    if (need_split) {  // need split
        ...
        for (int i = m; i < cnt; ++i) {  // move
            ...
            std::fill(page->records[i].key.begin(), page->records[i].key.end(),
                      0);
    ...
    if (root == page_addr) {  // update root
        if (update_new_root(page_addr, split_key, sibling_addr, level + 1, root,
            ...
            insert_internal(
                split_key, sibling_addr, cxt, coro_id,
                level + 1);  // BUG: 理论上不可能在root的分裂上有冲突吧?
            return true;
        }
    }
...
inline bool Tree::acquire_local_lock(GlobalAddress lock_addr,
    ...
    uint64_t lock_val =
        node.ticket_lock.fetch_add(1);  // 通过取值来依次排队, faa返回旧值
